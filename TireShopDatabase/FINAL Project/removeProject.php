<?php
  ...
  if (isset($_GET['Pnumber']) && isset($_GET['Pname']) && isset($_GET['Dnum']) && isset($_GET['Plocation'])) {
    ...
    echo '<p class="error">Sorry, no project was specified for removal.</p>';
  }
  ...
  if (isset($_POST['submit'])) {
    if ($_POST['confirm'] == 'Yes') {
      ...
      $query = "DELETE FROM A_PROJECT WHERE Pnumber = $Pnumber LIMIT 1";
      mysqli_query($dbc, $query);
      mysqli_close($dbc);

      // Confirm success with the user
      echo '<p> Project ' . $Pname . ' from department number ' . $Dnum . ' was successfully removed.';
    }
    else {
      echo '<p class="error">The project was not removed.</p>';
    }
  }
  else if (isset($Pnumber) && isset($Pname) && isset($Dnum) && isset($Plocation)) {
	
    echo '<p>Are you sure you want to delete the following project?</p>';
    echo '<p><strong>Project Name: </strong>' . $Pname . '<br /><strong>Department Number: </strong>' . $Dnum .
      '<br /> </p>';
    echo '<form method="post" action="removeProject.php">';
